...
x <- -1.1
test_that(paste("log1pdx2() warning for x = ", x), {
  testthat::expect_warning(log1pdx2(x))
})
res <- suppressWarnings(log1pdx2(x))
test_that(paste("log1pdx2() returns NaN for x = ", x), {
  testthat::expect_identical(res, NaN)
})

# x = -1: expect NaN (really Inf, but direct calculation results in NaN)
x <- -1
res <- log1pdx2(x)
test_that(paste("log1pdx2() returns NaN for x = ", x), {
  testthat::expect_identical(res, NaN)
...
res <- log1pdx2(x)
test_that(paste("log1pdx2() and log1pdx2(x) using logcf() agree for x = ", x), {
  testthat::expect_identical(res, 1 / 2)
})
